<?php
    ...
    if(!empty($_GET["cientificos"])){
        $cientificos=limpiaEntrada($_GET["cientificos"]);
            
    }else{
        $msgErr="El cientifico  es requerido";
        $error=true;
    }

    if(!$error){
        $resultado=get_coincidencias_cientificos($cientificos);
