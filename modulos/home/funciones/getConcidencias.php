<?php
    ...
    if(empty($_GET["inputBusqueda"])){
        $msgErr="El indice es requerido";
        $error=true;
      }else{
        $cadena=limpiaEntrada($_GET["inputBusqueda"]);
       
    }

    if(!$error){
        $resultado=get_coincidencias($cadena);
        ...
        if($resultado["estado"]==0){
            ...
            mysqli_free_result($articulos);  
                ...
                echo "<div class= 'card-body '>"; 
                echo "<h3 class= 'card-title'> <span class='fa fa-exclamation-triangle'></span></h3>"; 
                echo "<p class= 'text-left '>".$msgErr."</p>"; 
                echo "</div>"; 
                echo "</div>"; 
                echo "</div>"; 
                echo "</div>";  
